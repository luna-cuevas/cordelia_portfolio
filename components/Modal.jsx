...
const Modal = ( { openModal, setOpenModal, modalState, setModalState } ) => {
  ...
  return (
    <div className={`${modalState ? 'block': 'hidden'} md:inset-0 md:h-full fixed top-0 bottom-0 left-0 right-0 z-50 w-screen h-screen overflow-x-hidden overflow-y-auto`}>
      <button onClick={() => {setOpenModal(null); handlePlayVideo(); setModalState(false)}} className='absolute top-0 cursor-pointer bottom-0 left-0 z-0 right-0 w-screen h-screen bg-[#000000bc]'></button>
      <div className="w-fit relative z-20 flex h-screen p-4 m-auto">
        <div className="dark:bg-gray-700 relative m-auto bg-white rounded-lg shadow">
          ...
          <div className="relative z-20 flex pt-10 text-center">
            <button onClick={() => {handlePauseVideo()}} className='h-5/6 top-10 absolute left-0 right-0 z-20 w-full bg-transparent cursor-pointer' />
            <video ref={vidRef} className='md:max-h-[600px] cursor-grab h-2/3 m-auto' autoPlay controls src={openModal}></video>
          </div>
        </div>
      </div>      
    </div>
