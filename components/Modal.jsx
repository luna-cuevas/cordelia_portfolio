...
const Modal = ({openModal,setOpenModal, modalState, setModalState}) => {
  ...
  return (
    <div className={`${modalState ? 'block': 'hidden'} md:inset-0 md:h-full fixed top-0 bottom-0 left-0 right-0 z-50 w-screen h-screen overflow-x-hidden overflow-y-auto`}>
      <div onClick={() => {setOpenModal(null); handlePlayVideo(); setModalState(false)}} className='absolute top-0 bottom-0 left-0 right-0 w-screen h-screen bg-[#040202bc]'></div>
      <div className="w-fit relative flex h-screen p-4 m-auto">
        <div className="dark:bg-gray-700 relative m-auto bg-white rounded-lg shadow">
          <button onClick={() => {setOpenModal(null); handlePlayVideo(); setModalState(false)}} type="button" className="absolute top-0 right-1 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-lg p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray-800 dark:hover:text-white" >
          ...
          <div className="flex pt-10 text-center">
            <video ref={vidRef} className='md:max-h-[600px] h-2/3 m-auto' autoPlay controls src={openModal}></video>
          </div>
        </div>
      </div>      
    </div>
